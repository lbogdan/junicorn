...
export class AddProductComponent implements OnInit {
  ...
  validateFields(title, description, price) {
    if (!title) {
      const snackBarRef = this.snackBar.open('You must add a title for this product', 'OK!', {
        duration: 3000
      });
    } else if (!description) {
      const snackBarRef = this.snackBar.open('You must add a description to the product', 'OK!', {
        duration: 3000
      });
    } else if (!price) {
      const snackBarRef = this.snackBar.open('You must add a price to the product', 'OK!', {
        duration: 3000
      });
    }
  }
  ...
  addProduct(newTitle: string, newPrice: string, newCategory: any, newWeight: number, newDescription: string, newPublished: boolean) {
    ...
    if (newTitle && newPrice && newDescription && this.currentAdmin.uid) {
      ...
      const snackBarRef = this.snackBar.open('Product saved', 'OK!', {
        duration: 3000
      });
    }
  ...
  submitForModeration(newTitle: string, newPrice: string, newCategory: any,
    newWeight: number, newDescription: string, newPublished: boolean) {
    if (!newPublished) {
      newPublished = false;
    }
    ...
    if (newTitle && newPrice && newDescription && this.currentAdmin.uid) {
      ...
      if (this.editMode && this.productKey) {
        ...
        const adminApprovalProducts = this.db.list('/approvals/products/', ref => ref.orderByChild('updatedBy')
      ...
      const snackBarRef = this.snackBar.open('Product submitted for moderation', 'OK!', {
        duration: 3000
      });
  ...
  approveItem(newTitle: string, newPrice: string, newCategory: any, newDescription: string, newPublished: boolean) {
    if (this.entityObject.entityKey) {
      const ogEntity = this.db.object('/products/' + this.entityObject.entityKey);
    ...
    const snackBarRef = this.snackBar.open('Product approved', 'OK!', {
      duration: 3000
    });
